...
import {Island, Title, Button, Input, App} from './';
...
class MobilePaymentContract extends Component {
	...
	handleSubmit(event) {
		...
		axios({
			method: 'post',
			url: `/cards/${this.props.activeCard.id}/pay`,
			data: {
				type: "paymentMobile",
				data: phoneNumber,
				sum: Number(commission) + Number(sum)
			}
		})
			.then((response) => {
				this.props.activeCard.balance = response.data.card.balance;
				this.props.onPaymentSuccess({sum, phoneNumber, commission},response.data.transaction);
			});
	}
