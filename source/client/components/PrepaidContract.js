...
class PrepaidContract extends Component {
	...
	onSubmitForm(event) {
		...
		axios({
			...
			.then((response) => {
				this.props.activeCard.balance = response.data.balance1;
				this.props.inactiveCardsList[this.state.activeCardIndex].balance = response.data.balance2;

				const transactions = [response.data.transaction1,response.data.transaction2];
